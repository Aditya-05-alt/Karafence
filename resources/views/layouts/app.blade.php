...
<body class="bg-background-light dark:bg-background-dark text-secondary dark:text-gray-200 font-body transition-colors duration-300 flex flex-col min-h-screen">
    ...
    @if($whatsappUrl)
        <a href="{{ $whatsappUrl }}"
            ...
            class="whatsapp-float group fixed bottom-5 right-5 z-[9990] flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg hover:bg-[#20bd5a] hover:scale-110 transition-transform duration-200"
            ...
            <span class="absolute -top-10 right-0 whitespace-nowrap rounded-lg bg-secondary px-3 py-1.5 text-xs font-bold text-white opacity-0 pointer-events-none transition-opacity group-hover:opacity-100">Chat on WhatsApp</span>
        </a>
    @endif
